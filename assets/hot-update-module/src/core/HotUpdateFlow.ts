...
export default class HotUpdateFlow {
    ...
    async checkHotUpdate(): Promise<boolean> {
        ...
        if (!manifestMgr.needUpdate(localVersionManifest.version, remoteVersionManifest.version)) {
    ...
    async hotUpdateFlow(): Promise<{ success: boolean, error?: string }> {
        ...
        // 下载：获取需要下载的总字节数
        const totalBytes = manifestMgr.getTotalBytesToDownload(remoteProjectManifest);        
        const results = await downloader.downloadAssets(tasks, totalBytes); 
        ...
        if (this.m_isRetry) {
            ...
            if (failedTasks.length > 0) {
                const retryTasks = downloader.createRetryDownloadTasks(failedTasks);
                const retryResults = await downloader.retryFailedDownloads(retryTasks);
        ...
        // 验证：验证下载的资源文件
        const allVerified = verifier.verifyAssets(results);
        if (!allVerified) {
